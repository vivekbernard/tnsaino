...
public class CandidateCrudApiHandler {
    ...
    private static final Set<String> HANDLED_ROUTES = Set.of(
            ROUTE_PUT_CANDIDATE, ROUTE_GET_CANDIDATE, ROUTE_GET_CANDIDATE_LIST,
        ...
        return switch (routeKey(method, path)) {
            case ROUTE_PUT_CANDIDATE -> {
                if (!"CANDIDATE".equals(role) && !"ADMIN".equals(role))
                    yield responseFactory.forbidden("Candidate or Admin access required");
                yield handlePut(event);
            }
            case ROUTE_GET_CANDIDATE -> handleGet(event);
            case ROUTE_GET_CANDIDATE_LIST -> {
                if (!"ADMIN".equals(role)) yield responseFactory.forbidden("Admin access required");
                yield handleList(event);
            }
            case ROUTE_DELETE_CANDIDATE -> {
                if (!"ADMIN".equals(role)) yield responseFactory.forbidden("Admin access required");
                yield handleDelete(event);
            }
            case ROUTE_PUT_DISABLE -> {
                if (!"ADMIN".equals(role)) yield responseFactory.forbidden("Admin access required");
                yield handleDisable(event);
            }
            case ROUTE_PUT_ENABLE -> {
                if (!"ADMIN".equals(role)) yield responseFactory.forbidden("Admin access required");
                yield handleEnable(event);
            }
